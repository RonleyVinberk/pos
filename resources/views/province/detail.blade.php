@extends('layouts.app')
@section('title', 'POS-SMART | ' .$province->name)
@section('province_content')
{{-- start right_col --}}
<div class="right_col" role="main">

    {{-- start row --}}
    <div class="row">
        
        {{-- start col --}}
        <div class="col-xs-12 col-sm-12 col-md-12">

            {{-- start breadcrumb --}}
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb" style="background: #e9ecef !important;">
                    <li class="breadcrumb-item"><a class="white-text" href="{{route('homes.index')}}">Home</a></li>
                    <li class="breadcrumb-item">Master Data</li>
                    <li class="breadcrumb-item"><a href="{{route('provinces.index')}}">Provinces</a></li>
                    <li class="breadcrumb-item">{{$province->name}}</li>
                </ol>
            </nav>
            {{-- end breadcrumb --}}

            <!-- Notification -->
            @if (session('notification_error'))
                <div class="alert alert-danger">
                    {!! session('notification_error') !!}
                </div>
            @endif
            @if (session('notification_success'))
                <div class="alert alert-success">
                    {!! session('notification_success') !!}
                </div>
            @endif
                
            <a href="{{route('provinces.index')}}" class="btn btn-lg btn-default" role="button">Back</a>
            <a href="{{route('provinces.edit', $province->id)}}" class="btn btn-lg btn-primary" role="button">Edit</a>
            {{Form::open(['route' => ['provinces.destroy', $province->id], 'class' => 'form-horizontal', 'style' => 'display: -webkit-inline-box'])}}
            {{method_field('DELETE')}}
            {{Form::button('Delete', ['type' => 'submit', 'class' => 'btn btn-lg btn-danger', 'style' => 'position: relative; top: -15px;', 'onclick' => 'return confirm("You sure to delete this data?")'])}}
            {{Form::close()}}
    
            {{-- start x_panel --}}
            <div class="x_panel">
        
                {{-- start x_title --}}
                <div class="x_title">
        
                    <h4>Province Information</h4>
                            
                    <div class="clearfix"></div>
                            
                </div>
                {{-- end x_title --}}
        
                {{-- start x_content --}}
                <div class="x_content">
        
                    <table class="table table-striped table-bordered dt-responsive">
                        <tr>
                            <td style="text-align: right">Code</td>
                            <td>{{$province->code}}</td>
                        </tr>
                        <tr>
                            <td style="text-align: right; width: 45%">Name</td>
                            <td>{{$province->name}}</td>
                        </tr>
                        <tr>
                            <td style="text-align: right; width: 45%">Country</td>
                            <td>{{$province->country->name}} ({{$province->country->code}})</td>
                        </tr>
                    </table>
        
                </div>
                {{-- end x_content --}}
        
            </div>
            {{-- end x_panel --}}

            {{-- start x_panel --}}
            <div class="x_panel">
        
                {{-- start x_title --}}
                <div class="x_title">
            
                    <h4>Province Information II</h4>
                                
                    <div class="clearfix"></div>
                                
                </div>
                {{-- end x_title --}}

                {{-- start x_content --}}
                <div class="x_content">
                
                    <table class="table table-striped table-bordered dt-responsive">
                        <tr>
                            <td style="text-align: right; width: 45%">Created at</td>
                            <td>{{$province->created_at}}</td>
                        </tr>
                        <tr>
                            <td style="text-align: right">Updated at</td>
                            <td>{{$province->updated_at}}</td>
                        </tr>
                    </table>
            
                </div>
                {{-- end x_content --}}
            
            </div>
            {{-- end x_panel --}}
        
        </div>
        {{-- end col --}}
        
    </div>
    {{-- end row --}}
        
</div>
{{-- end right_col --}}
@endsection